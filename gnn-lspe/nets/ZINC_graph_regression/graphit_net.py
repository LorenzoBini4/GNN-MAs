    ...
    def __init__(self, net_params):
        ...
        if self.pe_init == 'rand_walk':
            ...
            self.layers = nn.ModuleList([ GraphiT_GT_LSPE_Layer(gamma, GT_hidden_dim, GT_hidden_dim, GT_n_heads, full_graph, dropout,
                                                                self.layer_norm, self.batch_norm, self.residual, self.adaptive_edge_PE, use_bias=self.use_bias) for _ in range(GT_layers-1) ])
            self.layers.append(GraphiT_GT_LSPE_Layer(gamma, GT_hidden_dim, GT_out_dim, GT_n_heads, full_graph, dropout,
                                                     self.layer_norm, self.batch_norm, self.residual, self.adaptive_edge_PE, use_bias=self.use_bias))
        else: 
            # NoPE
            self.layers = nn.ModuleList([ GraphiT_GT_Layer(gamma, GT_hidden_dim, GT_hidden_dim, GT_n_heads, full_graph, dropout,
                                                           self.layer_norm, self.batch_norm, self.residual, self.adaptive_edge_PE, use_bias=self.use_bias, explicit_bias=explicit_bias) for _ in range(GT_layers-1) ])
            self.layers.append(GraphiT_GT_Layer(gamma, GT_hidden_dim, GT_out_dim, GT_n_heads, full_graph, dropout,
                                                self.layer_norm, self.batch_norm, self.residual, self.adaptive_edge_PE, use_bias=self.use_bias, explicit_bias=explicit_bias))
