    ...
    def __init__(self, net_params):
        ...
        if self.pe_init == 'rand_walk':
            ...
            self.layers = nn.ModuleList([ SAN_GT_LSPE_Layer(self.gamma, GT_hidden_dim, GT_hidden_dim, GT_n_heads, full_graph,
                                                                dropout, self.layer_norm, self.batch_norm, self.residual, use_bias=use_bias, explicit_bias=explicit_bias) for _ in range(GT_layers-1) ])
            self.layers.append(SAN_GT_LSPE_Layer(self.gamma, GT_hidden_dim, GT_out_dim, GT_n_heads, full_graph,
                                                     dropout, self.layer_norm, self.batch_norm, self.residual, use_bias=use_bias, explicit_bias=explicit_bias))
        else: 
            # NoPE
