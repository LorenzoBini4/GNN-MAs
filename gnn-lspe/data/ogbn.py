...
class OGBNDGL(torch.utils.data.Dataset):
...
class OGBNDataset(Dataset):
    def __init__(self, name, features='full'):
        ...
        self.dataset = DglNodePropPredDataset(name=self.name)
        data = [self.dataset[i] for i in range(len(self.dataset))]

        if('feat' not in self.dataset[0][0].ndata.keys() or len(self.dataset[0][0].ndata['feat']) == 0):
            for g in data:
                self._node_species_to_feat(g[0]) # NOTE: in case of more graphs, the set of species should be pre-computed
                del g[0].ndata['species']
        self.num_atom_type = torch.cat([g[0].ndata['feat'] for g in data]).unique().shape[0]
        self.num_bond_type = g[0].edata['feat'].shape[1]
        ...
        self.train = OGBNDGL(self.dataset, split_idx['train'])
        self.val = OGBNDGL(self.dataset, split_idx['valid'])
        self.test = OGBNDGL(self.dataset, split_idx['test'])
    ...
    def _add_node_features(self, g):
        nfeat = []
        for i in tqdm(range(g.number_of_nodes())):
            nfeat.append((g.edata['feat'][g.edges()[1] == i]).mean(axis=0))
        r = torch.stack(nfeat)
        g.ndata['feat'] = r

    def _node_species_to_feat(self, g, species_key='species'):
        nfeat = g.ndata[species_key].detach().clone()
        keys = g.ndata[species_key].unique()
