...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def propagate_attention_explicit_bias(self, g):
        ...
        # Use available edge features to modify the scores for edges
        g.apply_edges(imp_exp_attn_bias('score', 'E', self.e_bias), edges=real_ids)
        
        if self.full_graph:
            g.apply_edges(imp_exp_attn_bias('score', 'E_2', self.e_bias_2), edges=fake_ids)
...
class GraphiT_GT_Layer(nn.Module):
    ...
    def forward(self, g, h, p, e, snorm_n):
        ...
        self.malog_e_attack = e_attn_out
        if self.malog:
