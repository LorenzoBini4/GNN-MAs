...
def src_dot_dst(src_field, dst_field, out_field):
    def func(edges):
        # print('edges.src.keys()', edges.src.keys())
        # print('edges.dst.keys()', edges.dst.keys())
...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, gamma, in_dim, out_dim, num_heads, full_graph, use_bias, adaptive_edge_PE, attention_for, explicit_bias=False):
        super().__init__()
    ...
    def propagate_attention(self, g):
        ...
        e_attn = g.edata['score'].detach().clone()
    ...
    def forward(self, g, h, e):
        ...
        self._update_ndata(g, 'Q_h', Q_h.view(-1, self.num_heads, self.out_dim))
        self._update_ndata(g, 'K_h', K_h.view(-1, self.num_heads, self.out_dim))
        g.edata['E'] = E.view(-1, self.num_heads, self.out_dim)
        
        
        if self.full_graph:
            g.ndata['Q_2h']['_N'] = Q_2h.view(-1, self.num_heads, self.out_dim)
            g.ndata['K_2h']['_N'] = K_2h.view(-1, self.num_heads, self.out_dim)
            g.edata['E_2'] = E_2.view(-1, self.num_heads, self.out_dim)
        
        self._update_ndata(g, 'V_h', V_h.view(-1, self.num_heads, self.out_dim))
        ...
        if self.explicit_bias:
            e_attn = self.propagate_attention_explicit_bias(g)
        else:
            e_attn = self.propagate_attention(g)
        
        h_out = g._node_frames[1]['wV'] / (g._node_frames[1]['z'] + torch.full_like(g._node_frames[1]['z'], 1e-6))
...
class GraphiT_GT_Layer(nn.Module):
    ...
    def __init__(self, gamma, in_dim, out_dim, num_heads, full_graph, dropout=0.0,
                 layer_norm=False, batch_norm=True, residual=True, adaptive_edge_PE=False, use_bias=False, explicit_bias=False):
        super().__init__()
        ...
        self.attention_h = MultiHeadAttentionLayer(gamma, in_dim, out_dim//num_heads, num_heads,
                                                   full_graph, use_bias, adaptive_edge_PE, attention_for="h", explicit_bias=explicit_bias)
