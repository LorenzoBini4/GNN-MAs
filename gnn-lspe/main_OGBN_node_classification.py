...
from nets.OGBN_node_classification.load_net import gnn_model # import GNNs
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs, train_bool=True, weights=None, epoch=0):
    ...
    from train.train_OGBN_node_classification import train_epoch_sparse as train_epoch, evaluate_network_sparse as evaluate_network

    num_workers = params['loader_workers']
    # neighbor_sampler = NeighborSampler(list(range(5,5+net_params['L']+1)))
    neighbor_sampler = NeighborSampler([5,10,15,1])
    train_loader = DataLoader(trainset[0][0], trainset.split, neighbor_sampler, num_workers=num_workers, batch_size=params['batch_size'], shuffle=True)#, pin_memory=True)
    val_loader = DataLoader(valset[0][0], valset.split, neighbor_sampler, num_workers=num_workers, batch_size=params['batch_size'], shuffle=False)#, pin_memory=True)
    test_loader = DataLoader(testset[0][0], testset.split, neighbor_sampler, num_workers=num_workers, batch_size=params['batch_size'], shuffle=False)#, pin_memory=True)
    test_log_loader = DataLoader(testset[0][0], testset.split, neighbor_sampler, num_workers=num_workers, batch_size=1, shuffle=False)
    ...
    try:
        with tqdm(range(epoch, params['epochs'])) as t:
            for epoch in t:
                ...
                processed = []
                epoch_train_loss, epoch_train_acc, optimizer = train_epoch(model, optimizer, device, train_loader, epoch, evaluator, trainset[0][1], processed=processed)
                
                epoch_val_loss, epoch_val_acc, __ = evaluate_network(model, device, val_loader, epoch, evaluator, valset[0][1])
                _, epoch_test_acc, __ = evaluate_network(model, device, test_loader, epoch, evaluator, testset[0][1])
                del __
                ...
                else:#if dataset.name == "ogbg-molpcba":
                    t.set_postfix(time=time.time()-start, lr=optimizer.param_groups[0]['lr'],
                                  train_loss=epoch_train_loss, val_loss=epoch_val_loss,
    ...
    # ___, __, g_outs_train = evaluate_network(model, device, train_loader, epoch, evaluator, trainset[0][1])
    val_loss, val_acc, g_outs_val = evaluate_network(model, device, val_loader, epoch, evaluator, valset[0][1])
    model.malog(True)
    test_loss, test_acc, g_outs_test = evaluate_network(model, device, test_log_loader, epoch, evaluator, testset[0][1], use_tqdm=True)
    # del ___
    # del __
    ...
    print("Val ACC: {:.4f}".format(val_acc), end=' - ')
