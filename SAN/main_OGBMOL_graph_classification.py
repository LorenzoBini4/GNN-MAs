...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs, train_bool=True, weights=None, epoch=0):
    ...
    if params['attack'] == -1: # perform usual tests without attack
        test_acc = 0.0
        ___, __, g_outs_train = evaluate_network(model, device, train_loader, epoch, evaluator)
        val_loss, val_acc, g_outs_val = evaluate_network(model, device, val_loader, epoch, evaluator)
        model.malog(True) # enable activation logging
        test_loss, test_acc, g_outs_test = evaluate_network(model, device, test_log_loader, epoch, evaluator)
        del ___
        del __
        ...
        if dataset.name == "ogbg-moltox21":
            ...
            Convergence Time (Epochs): {:.4f}\nTotal Time Taken: {:.4f} hrs\nAverage Time Per Epoch: {:.4f} s\n\n\n"""\
                .format(DATASET_NAME, MODEL_NAME, params, net_params, model, net_params['total_param'],
                        test_acc, train_acc, val_acc, epoch, (time.time()-t0)/3600, np.mean(per_epoch_time)))
    else: # inject noise and perform attack
        model.malog(False) # do not log activations
        def _freeze_r(p):
            for x in p:
                try:
        ...
        sample_loss, sample_mae, _ = evaluate_network(model, device, noise_loader, epoch, evaluator)
        ...
        for dev in devs:
            ...
            for _ in range(1):
                ...
                for noise_epoch in tqdm(list(range(1000))):
                    epoch_train_loss, epoch_train_mae, noise_optimizer = train_epoch(model, noise_optimizer, device, noise_loader, noise_epoch, evaluator, train_loss_function=noise_loss_function, zero_grad=noise_zero_grad, model_train=lambda m:m.eval())
                    noise_scheduler.step(epoch_train_loss)
                    if noise_optimizer.param_groups[0]['lr'] < params['min_lr']:
                        break
                sample_loss, sample_mae, _ = evaluate_network(model, device, noise_loader, noise_epoch, evaluator)
...
def main():    
    ...
    spec = [
        ...
        "SAN",
        dataset.name, 
        "full" if net_params.get("full_graph",False) else "sparse",
        net_params['LPE'],
        "BN" if net_params.get("batch_norm",True) else "LN",
        "ExplicitBias" if net_params.get("explicit_bias",False) else "NoBias",
        "Edge" if net_params.get("edge_feat",True) else "NoEdge",
        "base" if params.get("epochs",1000) == 0 else "train"
    ]
    for d,x in zip(
        chklogs.values(),
        [
            os.path.join('..', 'SAN', os.path.join(root_log_dir, 'RUN_0', 'malog.pkl')), 
            os.path.join('..', 'SAN', root_ckpt_dir),
            str(config)
        ]
    ):
        for sp in spec[:-1]:
