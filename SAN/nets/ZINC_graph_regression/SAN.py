...
class SAN(nn.Module):
    def __init__(self, net_params):
        ...
        self.layers = nn.ModuleList([ GraphTransformerLayer(gamma, GT_hidden_dim, GT_hidden_dim, GT_n_heads, full_graph, dropout, self.layer_norm, self.batch_norm, self.residual, use_bias=self.use_bias, explicit_bias=self.explicit_bias) for _ in range(GT_layers-1) ])
