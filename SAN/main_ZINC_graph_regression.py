...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs, train_bool=True, weights=None, epoch=0):
    ...
    _, train_mae = evaluate_network(model, device, train_loader, epoch, net_params['LPE'])
    val_loss, val_mae = evaluate_network(model, device, val_loader, epoch, net_params['LPE'])
    model.malog(True) # enable activation logging
    test_loss, test_mae = evaluate_network(model, device, test_log_loader, epoch, net_params['LPE'])
...
def main():    
    ...
    net_params['explicit_bias'] = True if (args.explicit_bias is not None and args.explicit_bias=='True') else False
