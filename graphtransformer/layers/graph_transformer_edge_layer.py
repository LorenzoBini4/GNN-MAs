...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def propagate_attention_explicit_bias(self, g):
        ...
        # Use available edge features to modify the scores
        g.apply_edges(imp_exp_attn_bias('score', 'proj_e', self.e_bias))
        ...
        # Send weighted values to target nodes
        eids = g.edges()
        g.send_and_recv(eids, src_mul_edge_bias('V_h', 'score', 'V_h', self.v_bias), fn.sum('V_h', 'wV'))
        g.send_and_recv(eids, fn.copy_edge('score', 'score'), fn.sum('score', 'z'))
    ...
    def forward(self, g, h, e):
        ...
        K_h = self.K(h)
        V_h = self.V(h)
        Q_h = Q_h.view(*(Q_h.shape[:-1]), self.num_heads, self.out_dim)
        K_h = K_h.view(*(K_h.shape[:-1]), self.num_heads, self.out_dim)
        V_h = V_h.view(*(V_h.shape[:-1]), self.num_heads, self.out_dim)
        if self.edge_feat:
...
class GraphTransformerLayer(nn.Module):
    """
        Param: 
    """
    def __init__(self, in_dim, out_dim, num_heads, dropout=0.0, layer_norm=False, batch_norm=True, residual=True, use_bias=False, explicit_bias=False, edge_feat=True, O_linear=True):
        super().__init__()
    ...
    def forward(self, g, h, e):
        ...
        if self.edge_feat:
            e = e_attn_out.reshape(-1, self.out_channels)
        if self.malog:
            malog_h['attention'] = (h.to('cpu'))
            malog_e['attention'] = (e.to('cpu'))
        self.malog_e_attack = e
